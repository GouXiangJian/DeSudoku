...
#Compile : perlapp --icon log.ico -g -f DeSudoku.pl -e=DeSudoku.exe
...
sub BuildGui {
    ...
    $MainWin->geometry("620x560");

    $MainWin->title("DeSudoku");

    my $tool = $MainWin->Frame()->pack(-side => 'top', -fill => 'x');
    $tool->Label(-text => " DeSudoku : A fast tool for decoding standard 9x9 Sudoku. ")->pack();
    ...
    my $qq = $MainWin->Frame()->pack(-side => 'top', -fill => 'x');
    $qq->Label(-text => " QQ : 862137261 ")->pack();
    ...
    my $MainFrame1 = $MainWin->Frame()->pack(-side => 'top', -fill => 'x');
    $MainFrame1->Label(-text => '')->pack(-ipadx => 40, -side => "left");
    $MainFrame1->Button(-text => "Example1", -command => \&Example1)->pack(-ipadx => 30, -side => "left");
    $MainFrame1->Label(-text => '')->pack(-ipadx => 20, -side => "left");
    $MainFrame1->Button(-text => "Example2", -command => \&Example2)->pack(-ipadx => 30, -side => "left");
    $MainFrame1->Label(-text => '')->pack(-ipadx => 20, -side => "left");
    $MainFrame1->Button(-text => "Example3", -command => \&Example3)->pack(-ipadx => 30, -side => "left");

    my $null6 = $MainWin->Frame()->pack(-side => 'top', -fill => 'x');
    $null6->Label(-text => '')->pack();

    my $MainFrame2 = $MainWin->Frame()->pack(-side => 'top', -fill => 'x');
    $MainFrame2->Label(-text => '')->pack(-ipadx => 40, -side => "left");
    $MainFrame2->Button(-text => "Clear", -command => \&Clear)->pack(-ipadx => 41, -side => "left");
    $MainFrame2->Label(-text => '')->pack(-ipadx => 20, -side => "left");
    $MainFrame2->Button(-text => "Start", -command => \&DeSudoku)->pack(-ipadx => 41, -side => "left");
    $MainFrame2->Label(-text => '')->pack(-ipadx => 20, -side => "left");
    $MainFrame2->Button(-text => "Close", -command => sub{exit})->pack(-ipadx => 41, -side => "left");

    my $null7 = $MainWin->Frame()->pack(-side => 'top', -fill => 'x');
    $null7->Label(-text => '')->pack();
...
sub DeSudoku {
    ...
    my $RunTime = $EndTime - $StartTime < 0 ? 0 : $EndTime - $StartTime;
    $RunTime = sprintf "%.3f s", $RunTime;
...
sub Example1 {
    ...
    foreach my $i (1 .. 9) {
        foreach my $j (1 .. 9) {
            $entry{$i}{$j}->delete('0.0', 'end');
            $entry{$i}{$j}->insert('end', $test[$i-1][$j-1]);
        }
    }
}
